...
@section('content')
<main class="max-w-5xl mx-auto p-4 sm:p-5 lg:p-10 space-y-8 animate-in">
    ...
    <section class="bg-white rounded-3xl sm:rounded-[3rem] p-6 sm:p-8 border border-maroon-50 shadow-sm flex flex-col md:flex-row items-center gap-6 sm:gap-8">
        <div class="w-20 h-20 sm:w-24 sm:h-24 rounded-2xl sm:rounded-3xl bg-slate-100 overflow-hidden border-4 border-maroon-50 shadow-inner shrink-0 flex items-center justify-center text-3xl font-black text-maroon-900">
        ...
        <div class="flex-1 text-center md:text-left">
            ...
            <h2 class="text-2xl sm:text-3xl font-black text-maroon-950 italic tracking-tight">{{ $siswa->nama_lengkap ?? 'Nama Siswa' }}</h2>
            <div class="flex flex-wrap justify-center md:justify-start items-center gap-2 sm:gap-4 mt-2 sm:mt-3">
                <span class="text-xs sm:text-sm font-bold text-slate-400">NIS: {{ $siswa->nis ?? '-' }}</span>
                <span class="w-1 h-1 sm:w-1.5 sm:h-1.5 bg-maroon-100 rounded-full"></span>
                <span class="text-xs sm:text-sm font-bold text-slate-400 uppercase tracking-tighter">{{ $siswa->sekolah_asal ?? 'Instansi Tidak Diketahui' }}</span>
            </div>
        </div>
        <div class="bg-maroon-50 w-full md:w-auto px-6 sm:px-8 py-4 rounded-2xl sm:rounded-[2rem] border border-maroon-100 text-center shrink-0 shadow-sm">
            <p class="text-[9px] sm:text-[10px] font-black text-maroon-400 uppercase tracking-widest mb-1 leading-none">Menunggu Review</p>
            <p class="text-2xl sm:text-3xl font-black text-amber-500 leading-none">
                {{ collect($riwayatLog ?? [])->where('status', 'pending')->count() }}
            </p>
        </div>
    </section>

    <!-- DAFTAR LOGBOOK & FILTER -->
    <section class="space-y-6">
        
        <!-- HEADER SECTION DENGAN FILTER -->
        <div class="flex flex-col xl:flex-row xl:items-center justify-between gap-4 px-2">
            <h3 class="text-lg sm:text-xl font-black text-maroon-950 tracking-tight italic">Laporan Aktivitas Harian</h3>
            
            <!-- FORM FILTER BAR (Ditambahkan ID dan onsubmit JS) -->
        ...
        <div id="logbook-container" class="space-y-6 sm:space-y-8">
            @forelse($riwayatLog ?? [] as $log)
                @php
                    ...
                    // Mendapatkan nilai bulan dan tahun murni untuk kebutuhan filter JS
                ...
                <!-- LOG CARD DENGAN DATA ATRIBUT FILTER -->
                <div class="log-card bg-white rounded-3xl sm:rounded-[2.5rem] p-5 sm:p-8 border border-maroon-50 relative overflow-hidden group border-l-[6px] sm:border-l-8 {{ $borderColor }} {{ $cardOpacity }} transition-all duration-300"
                    ...
                    <div class="absolute top-4 sm:top-8 right-4 sm:right-8 px-3 py-1 sm:px-4 sm:py-1.5 border rounded-full text-[8px] sm:text-[9px] font-black uppercase tracking-widest {{ $badgeBg }}">
                        {{ $badgeText }}
                    </div>

                    <div class="space-y-4 sm:space-y-6 mt-6 sm:mt-0">
                        <!-- Tanggal / Header Log -->
                        ...
                        <div class="space-y-2">
                            <p class="text-[8px] sm:text-[9px] font-black text-maroon-900 uppercase tracking-widest ml-1 leading-none">Isi Laporan Siswa:</p>
                            <div class="bg-slate-50 p-4 sm:p-6 rounded-2xl border border-slate-100 italic font-medium text-slate-600 leading-relaxed text-xs sm:text-sm shadow-inner">
                        ...
                        @if($isPending)
                            <form action="{{ route('pembimbing.monitoring.validasi', $log->id_log) }}" method="POST" class="space-y-4 pt-4 border-t border-slate-50">
                                ...
                                <div class="space-y-3">
                                    <label class="text-[8px] sm:text-[9px] font-black text-maroon-900 uppercase tracking-widest ml-1 flex items-center gap-2">
                                    ...
                                    <textarea name="catatan_pembimbing" placeholder="Berikan catatan perbaikan atau apresiasi kepada siswa..." rows="3" class="w-full bg-maroon-50/30 border border-maroon-100/50 rounded-xl sm:rounded-2xl px-4 sm:px-5 py-3 sm:py-4 text-xs sm:text-sm font-medium text-maroon-900 placeholder:text-maroon-300 focus:ring-2 focus:ring-maroon-500 focus:bg-white transition-all outline-none resize-none shadow-inner"></textarea>
                                </div>

                                <div class="flex flex-col sm:flex-row items-center gap-2 sm:gap-3">
                                    <button type="submit" name="status" value="diterima" class="w-full sm:flex-1 bg-emerald-500 text-white py-3.5 sm:py-4 rounded-xl sm:rounded-2xl text-[9px] sm:text-[10px] font-black uppercase tracking-[0.2em] shadow-lg shadow-emerald-500/20 active:scale-95 transition-all flex items-center justify-center gap-2 hover:bg-emerald-600">
                                    ...
                                    <button type="submit" name="status" value="ditolak" class="w-full sm:flex-1 bg-rose-50 text-rose-600 border border-rose-100 py-3.5 sm:py-4 rounded-xl sm:rounded-2xl text-[9px] sm:text-[10px] font-black uppercase tracking-[0.2em] active:scale-95 transition-all flex items-center justify-center gap-2 hover:bg-rose-100">
                        ...
                        @else
                            ...
                            @if(!empty($log->catatan_pembimbing))
                                ...
                                @endphp
                                <div class="{{ $noteBg }} p-3 sm:p-4 rounded-xl sm:rounded-2xl border flex gap-3 sm:gap-4 items-start mt-4">
            ...
            @empty
                <!-- Tampilan Apabila Data Dari Server Kosong -->
                <div class="bg-white/50 backdrop-blur-sm p-10 sm:p-12 rounded-3xl sm:rounded-[3rem] border-2 border-dashed border-maroon-100 flex flex-col items-center justify-center text-center">
                    ...
                    <p class="text-xs sm:text-sm font-medium text-slate-500 max-w-sm mx-auto">Siswa ini belum memiliki catatan aktivitas logbook harian.</p>
                </div>
            @endforelse
            
            <!-- EMPTY STATE JS (Ditampilkan oleh Javascript saat filter tidak cocok) -->
            <div id="js-empty-state" style="display: none;" class="bg-white/50 backdrop-blur-sm p-10 sm:p-12 rounded-3xl sm:rounded-[3rem] border-2 border-dashed border-maroon-100 flex-col items-center justify-center text-center">
                ...
                <p class="text-xs sm:text-sm font-medium text-slate-500 max-w-sm mx-auto">Tidak ada laporan yang sesuai dengan status atau bulan yang Anda pilih.</p>
            </div>
