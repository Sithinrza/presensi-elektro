...
<main class="max-w-7xl mx-auto p-4 sm:p-5 lg:p-10 animate-in">
    ...
    <section class="space-y-8 sm:space-y-10">
        ...
        <!-- HEADER & PENCARIAN (Desain Baru Minimalis) -->
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white/50 backdrop-blur-md p-4 sm:p-6 rounded-3xl sm:rounded-[2.5rem] border border-maroon-50/50 shadow-sm">
            <div class="flex items-center gap-3 sm:gap-4 pl-1 sm:pl-2">
                ...
                <div>
                    <h2 class="text-lg sm:text-xl font-black text-maroon-950 tracking-tight leading-none">Jurnal Kegiatan</h2>
                    <p class="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1.5 line-clamp-1">Validasi Logbook Harian Siswa</p>
                </div>
            </div>
        ...
        <!-- GRID KARTU SISWA (Desain Premium) -->
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 xl:gap-8">
            ...
            <div class="relative bg-white rounded-3xl sm:rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1.5 hover:border-maroon-100 transition-all duration-300 group flex flex-col overflow-hidden">
                ...
                <div class="px-5 pb-5 sm:px-6 sm:pb-6 relative flex flex-col flex-1">
                    ...
                    <div class="flex justify-between items-end -mt-8 sm:-mt-10 mb-4 sm:mb-5">
                        ...
                        <!-- Indikator Butuh Validasi -->
                        @if(($s->belum_divalidasi ?? 0) > 0)
                            <div class="mb-2 mr-1 px-3 py-1 bg-amber-50 border border-amber-200 text-amber-600 rounded-full text-[9px] font-black uppercase tracking-widest flex items-center gap-1.5 shadow-sm">
                    ...
                    <!-- Update Terakhir -->
                    <div class="mt-auto mb-6 p-4 bg-slate-50/70 rounded-2xl border border-slate-100/80">
                        <p class="text-[8px] sm:text-[9px] font-black text-slate-300 uppercase tracking-widest mb-2 flex items-center gap-1.5">
                        ...
                        @if($s->log_terakhir)
                            <p class="text-[10px] sm:text-[11px] font-bold text-slate-600 leading-snug italic line-clamp-2">"{{ $s->log_terakhir->description ?? $s->log_terakhir->uraian }}"</p>
                            <p class="text-[8px] sm:text-[9px] font-bold text-maroon-400 mt-2.5">{{ \Carbon\Carbon::parse($s->log_terakhir->report_date ?? $s->log_terakhir->tanggal)->translatedFormat('d M Y • H:i') }} WITA</p>
                        @else
                            <p class="text-[10px] sm:text-[11px] font-bold text-slate-400 leading-snug italic">Belum pernah mengisi logbook harian.</p>
                        @endif
                    </div>

                    <!-- Tombol Aksi -->
                    <a href="{{ route('pembimbing.monitoring.show', $s->id_siswa) }}" class="w-full bg-slate-50 border border-slate-100 text-slate-500 py-3.5 sm:py-4 rounded-xl sm:rounded-2xl text-[9px] sm:text-[10px] font-black uppercase tracking-[0.2em] flex items-center justify-center gap-2 group-hover:bg-maroon-950 group-hover:text-white group-hover:border-maroon-950 group-hover:shadow-lg group-hover:shadow-maroon-950/20 active:scale-95 transition-all duration-300 z-10">
                        Buka Detail Jurnal
            ...
            <div class="col-span-full bg-white/50 backdrop-blur-sm p-8 sm:p-12 rounded-3xl sm:rounded-[3rem] border-2 border-dashed border-maroon-100 flex flex-col items-center justify-center text-center">
                <div class="w-16 h-16 sm:w-20 sm:h-20 bg-white rounded-full shadow-sm flex items-center justify-center text-maroon-200 mb-4 sm:mb-6">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8 sm:w-10 sm:h-10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg>
                </div>
                <h3 class="text-lg sm:text-xl font-black text-maroon-950 tracking-tight mb-2">Tidak Ada Data</h3>
                <p class="text-xs sm:text-sm font-medium text-slate-500 max-w-md mx-auto">Saat ini belum ada data anak bimbingan yang terkait dengan akun Anda, atau pencarian Anda tidak menemukan hasil.</p>
            </div>
            @endforelse
