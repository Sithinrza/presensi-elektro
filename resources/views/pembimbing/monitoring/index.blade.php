...
<main class="max-w-7xl mx-auto p-4 sm:p-5 lg:p-10 animate-in">
    ...
    <section class="space-y-6 sm:space-y-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white/60 backdrop-blur-md p-4 sm:p-6 rounded-3xl sm:rounded-[2.5rem] border border-maroon-100/50 shadow-sm">
            ...
            <form action="{{ route('pembimbing.monitoring.index') }}" method="GET" class="flex flex-col sm:flex-row gap-2 w-full md:w-auto mt-2 md:mt-0">
                <select name="status" onchange="this.form.submit()" class="bg-white border-2 border-slate-100 rounded-xl sm:rounded-2xl px-4 py-3 sm:py-3.5 text-[11px] sm:text-xs font-bold text-maroon-950 shadow-sm hover:border-maroon-200 focus:border-maroon-500 outline-none cursor-pointer w-full sm:w-auto">
                ...
                <div class="px-5 pb-5 sm:px-6 sm:pb-6 relative flex flex-col flex-1">
                    ...
                    <div class="mb-5">
                        <div class="flex items-center gap-2 mb-1.5">
                            ...
                            @if($s->status == 'Aktif')
                                <span class="bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded text-[8px] sm:text-[9px] font-black uppercase tracking-widest shrink-0">Aktif</span>
                            @else
                                <span class="bg-rose-100 text-rose-700 px-2 py-0.5 rounded text-[8px] sm:text-[9px] font-black uppercase tracking-widest shrink-0">Selesai/Nonaktif</span>
                            @endif
                        </div>
                        <p class="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-[0.15em] truncate">{{ $s->sekolah_asal ?? 'Instansi Tidak Diisi' }}</p>
                    </div>
