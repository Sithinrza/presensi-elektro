...
<main class="max-w-7xl mx-auto p-4 sm:p-5 lg:p-10 space-y-6 sm:space-y-8 animate-in">
    ...
    <!-- KARTU PROFIL & STATISTIK (MINIMALIS) -->
    <section class="bg-white rounded-3xl sm:rounded-[2.5rem] p-5 sm:p-8 border border-slate-100 shadow-sm">
        <div class="flex flex-col lg:flex-row gap-6 lg:gap-8 items-start lg:items-center">
            ...
            <!-- Grid Statistik -->
            <div class="grid grid-cols-3 sm:grid-cols-5 gap-2 sm:gap-3 w-full lg:w-auto shrink-0 mt-4 lg:mt-0">
                <!-- Tepat CI -->
                <div class="bg-emerald-50/50 border border-emerald-100 p-3 sm:p-4 rounded-xl sm:rounded-2xl text-center flex flex-col justify-center">
                    <p class="text-xl sm:text-2xl font-black text-emerald-600 leading-none">{{ $statistik['Tepat CI'] }}</p>
                    <p class="text-[8px] sm:text-[9px] font-bold text-emerald-700/60 uppercase tracking-widest mt-1.5 line-clamp-1">Tepat CI</p>
                </div>
                <!-- Telat CI -->
                <div class="bg-amber-50/50 border border-amber-100 p-3 sm:p-4 rounded-xl sm:rounded-2xl text-center flex flex-col justify-center">
                    <p class="text-xl sm:text-2xl font-black text-amber-500 leading-none">{{ $statistik['Telat CI'] }}</p>
                    <p class="text-[8px] sm:text-[9px] font-bold text-amber-700/60 uppercase tracking-widest mt-1.5 line-clamp-1">Telat CI</p>
                </div>
                <!-- Alfa -->
                <div class="bg-rose-50/50 border border-rose-100 p-3 sm:p-4 rounded-xl sm:rounded-2xl text-center flex flex-col justify-center">
                    <p class="text-xl sm:text-2xl font-black text-rose-500 leading-none">{{ $statistik['Alfa'] }}</p>
                    <p class="text-[8px] sm:text-[9px] font-bold text-rose-700/60 uppercase tracking-widest mt-1.5 line-clamp-1">Alfa</p>
                </div>
                <!-- Tepat CO -->
                <div class="bg-emerald-50/50 border border-emerald-100 p-3 sm:p-4 rounded-xl sm:rounded-2xl text-center flex flex-col justify-center">
                    <p class="text-xl sm:text-2xl font-black text-emerald-600 leading-none">{{ $statistik['Tepat CO'] }}</p>
                    <p class="text-[8px] sm:text-[9px] font-bold text-emerald-700/60 uppercase tracking-widest mt-1.5 line-clamp-1">Tepat CO</p>
                </div>
                <!-- Lupa CO -->
                <div class="bg-rose-50/50 border border-rose-100 p-3 sm:p-4 rounded-xl sm:rounded-2xl text-center flex flex-col justify-center">
                    <p class="text-xl sm:text-2xl font-black text-rose-500 leading-none">{{ $statistik['Lupa CO'] }}</p>
                    <p class="text-[8px] sm:text-[9px] font-bold text-rose-700/60 uppercase tracking-widest mt-1.5 line-clamp-1">Lupa CO</p>
                </div>
            </div>
        </div>
    </section>
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        <td class="px-6 sm:px-8 py-4 sm:py-5">
                            <p class="font-bold text-slate-800 text-xs sm:text-sm">{{ \Carbon\Carbon::parse($p->tanggal)->translatedFormat('l, d M Y') }}</p>
                        </td>
                        <td class="px-6 sm:px-8 py-4 sm:py-5 text-center text-slate-500 text-xs sm:text-sm font-semibold">
                            {{ $p->jam_masuk ?? '--:--' }}
                        </td>
                        <td class="px-6 sm:px-8 py-4 sm:py-5 text-center text-slate-500 text-xs sm:text-sm font-semibold">
                            {{ $p->jam_pulang ?? '--:--' }}
