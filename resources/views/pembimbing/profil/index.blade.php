...
<main class="max-w-4xl mx-auto p-4 sm:p-5 lg:p-10 space-y-6 sm:space-y-8 animate-in">
    ...
    <div class="flex items-center justify-between mb-2 sm:mb-4 lg:hidden">
        <h1 class="text-lg sm:text-xl font-extrabold text-maroon-950 tracking-tight">Profil Pengguna</h1>
    ...
    <section class="bg-maroon-900 rounded-3xl sm:rounded-[3rem] p-6 sm:p-8 md:p-12 text-white shadow-premium border border-maroon-800 relative flex flex-col md:flex-row items-center justify-center md:justify-start gap-5 sm:gap-8">
        ...
        <div x-data="{ openPhotoMenu: false }" class="relative group shrink-0 z-10">
            <button type="button" @click="openPhotoMenu = !openPhotoMenu" class="relative w-24 h-24 sm:w-32 sm:h-32 md:w-40 md:h-40 rounded-full border-4 border-gold p-1 shadow-2xl bg-white focus:outline-none transition-transform active:scale-95 block">
                <div class="w-full h-full rounded-full overflow-hidden relative bg-slate-100 flex items-center justify-center font-black text-3xl sm:text-4xl text-maroon-900">
                    @if($pembimbing->foto_profil)
                        <img src="{{ asset('storage/' . $pembimbing->foto_profil) }}" alt="Foto Profil" class="w-full h-full object-cover">
                    @else
                        {{ substr($pembimbing->nama_lengkap, 0, 1) }}
                    @endif
                    ...
                    <div class="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity backdrop-blur-[2px]">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="text-white sm:w-[28px] sm:h-[28px]"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
                    </div>
                </div>

                <div class="absolute bottom-0 right-0 sm:bottom-1 sm:right-1 w-8 h-8 sm:w-10 sm:h-10 bg-gold text-maroon-950 rounded-full flex items-center justify-center border-4 border-maroon-900 shadow-lg pointer-events-none">
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" class="sm:w-[16px] sm:h-[16px]"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
                </div>
            </button>

            <!-- Dropdown Menu Foto (Kini tidak akan terpotong) -->
        ...
        <div class="relative z-10 text-center md:text-left space-y-1.5 sm:space-y-2">
            <h2 class="text-2xl sm:text-3xl md:text-4xl font-black tracking-tight leading-none italic">{{ $pembimbing->nama_lengkap }}</h2>
            <div class="flex flex-col sm:flex-row items-center justify-center md:justify-start gap-2 sm:gap-3 mt-1">
                <span class="text-gold font-bold text-[10px] sm:text-sm tracking-widest uppercase">{{ $pembimbing->jabatan ?? 'Pembimbing' }}</span>
                <span class="hidden sm:block text-maroon-300">•</span>
                <span class="text-maroon-100 text-[10px] sm:text-sm font-medium">NIP. {{ $pembimbing->no_induk ?? '-' }}</span>
            </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6 items-start">
        ...
        <section class="bg-white rounded-3xl sm:rounded-[2.5rem] p-6 sm:p-8 border border-maroon-100 shadow-sm">
            ...
            <a href="{{ route('pembimbing.profil.edit') }}" class="w-full flex items-center justify-between p-3.5 sm:p-4 bg-maroon-50 hover:bg-maroon-100 rounded-xl sm:rounded-2xl transition-all group">
        ...
        <div class="space-y-5 sm:space-y-6">
            <section class="bg-white rounded-3xl sm:rounded-[2.5rem] p-6 sm:p-8 border border-maroon-100 shadow-sm space-y-5 sm:space-y-6">
                ...
                <div class="space-y-3 sm:space-y-4">
                    <!-- Form Ubah Kata Sandi (Dengan Konfirmasi SweetAlert) -->
