...
<main class="max-w-4xl mx-auto p-4 sm:p-5 lg:p-10 space-y-6 sm:space-y-8 animate-in">
    ...
    <div class="flex items-center gap-3 sm:gap-4 border-b border-maroon-100/30 pb-3 sm:pb-4">
        <a href="{{ route('pembimbing.profil.index') ?? '#' }}" class="w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center rounded-lg sm:rounded-xl bg-maroon-50 text-maroon-950 hover:bg-maroon-100 transition-colors shadow-sm shrink-0">
    ...
    <form action="{{ route('pembimbing.profil.update') }}" method="POST" class="space-y-6 sm:space-y-8">
        ...
        <div class="flex flex-col-reverse sm:flex-row justify-end gap-3 sm:gap-4 bg-maroon-50/30 p-4 sm:p-6 rounded-2xl sm:rounded-[2rem] border border-maroon-50">
            <a href="{{ route('pembimbing.profil.index') ?? '#' }}" class="w-full sm:w-auto bg-white text-maroon-900 border border-maroon-100 py-3 sm:py-4 px-6 sm:px-10 rounded-xl sm:rounded-2xl font-black text-[10px] sm:text-xs uppercase tracking-widest text-center shadow-sm hover:bg-maroon-50 transition-all active:scale-95">Kembali</a>
