    ...
    <aside class="hidden lg:flex fixed left-0 top-0 h-full w-28 bg-maroon-950 flex-col items-center py-8 z-50 border-r border-white/5 shadow-2xl">
        ...
        <div class="flex flex-col gap-3 flex-1 w-full px-3">
            ...
            <a href="{{ route('siswa.dashboard') }}" title="Dashboard"
               class="group nav-transition flex flex-col items-center justify-center py-4 rounded-[2rem] {{ request()->routeIs('siswa.dashboard') || request()->is('siswa/dashboard') ? 'bg-white text-maroon-950 shadow-xl scale-105' : 'text-white/75 hover:bg-white/10 hover:text-white' }}">
            ...
            <a href="{{ route('presensi.index') }}" title="Presensi"
               class="group nav-transition flex flex-col items-center justify-center py-4 rounded-[2rem] {{ request()->routeIs('presensi.index') || request()->is('siswa/presensi') ? 'bg-white text-maroon-950 shadow-xl scale-105' : 'text-white/75 hover:bg-white/10 hover:text-white' }}">
                ...
                <span class="text-[9px] font-black uppercase tracking-[0.2em] mt-2 group-hover:opacity-100 transition-opacity {{ request()->routeIs('presensi.index') || request()->is('siswa/presensi') ? 'opacity-100' : 'opacity-80' }}">Presensi</span>
            </a>

            <!-- 3. Riwayat Presensi -->
            <a href="{{ route('presensi.riwayat-presensi') }}" title="Riwayat Presensi"
               class="group nav-transition flex flex-col items-center justify-center py-4 rounded-[2rem] {{ request()->routeIs('presensi.riwayat-presensi') || request()->is('siswa/riwayat*') ? 'bg-white text-maroon-950 shadow-xl scale-105' : 'text-white/75 hover:bg-white/10 hover:text-white' }}">
        ...
        <div class="w-full px-3 mt-4 space-y-2">
            <a href="{{ url('/siswa/profil') }}" title="Profil Saya"
               class="group nav-transition flex flex-col items-center justify-center py-4 rounded-[2rem] {{ request()->is('siswa/profil*') ? 'bg-white text-maroon-950 shadow-xl scale-105' : 'text-white/75 hover:text-white hover:bg-white/10' }}">
                <div class="relative">
                ...
                <span class="text-[9px] font-black uppercase tracking-[0.2em] mt-2 group-hover:opacity-100 transition-opacity {{ request()->is('siswa/profil*') ? 'opacity-100' : 'opacity-80' }}">Profil</span>
            </a>
        </div>
    </aside>