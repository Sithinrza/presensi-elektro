        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="hidden md:block text-[10px] font-bold text-gold-dark uppercase tracking-widest mt-1">
                    {{ $profil->sekolah_asal ?? 'Siswa Magang' }} • {{ $profil->jurusan ?? 'Elektro' }}
                </p>
            </div>
        </div>
        <div class="flex items-center gap-3">
            ...
            <div x-data="{ open: false }" class="relative">
                <button @click="open = !open" class="w-10 h-10 rounded-full border-2 border-gold p-0.5 block focus:outline-none active:scale-95 transition-transform overflow-hidden bg-white">
                    @if($profil && $profil->foto_profil)
                        <img src="{{ asset('uploads/profil/' . $profil->foto_profil) }}" alt="Profile" class="w-full h-full rounded-full object-cover">
                    @else
