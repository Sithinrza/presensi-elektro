...
<header class="sticky top-0 z-40 glass-effect border-b border-maroon-100/30 px-4 sm:px-5 lg:px-10 py-3 sm:py-4 bg-white/80 backdrop-blur-md">
    <div class="max-w-7xl mx-auto flex items-center justify-between gap-4">
        <div class="flex items-center gap-3 overflow-hidden">
            ...
            <div class="min-w-0"> <h1 class="text-sm sm:text-base md:text-lg font-extrabold text-maroon-950 leading-none tracking-tight truncate">
                    {{ $profil->nama_lengkap ?? Auth::user()->email }}
                </h1>
                <p class="hidden md:block text-[10px] font-bold text-gold-dark uppercase tracking-widest mt-1.5 truncate">
                    Siswa Magang • {{ $profil->sekolah_asal ?? 'Teknik Elektro' }}
                </p>
            </div>
        </div>

        <div class="flex items-center gap-2 sm:gap-3 shrink-0">
            <button class="relative w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center rounded-lg sm:rounded-xl bg-white border border-maroon-100 shadow-sm text-maroon-900 active:scale-90 transition">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 sm:w-5 sm:h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9"/><path d="M10.3 21a1.94 1.94 0 0 0 3.4 0"/></svg>
                <span class="absolute top-1.5 right-1.5 sm:top-2.5 sm:right-2.5 w-2 h-2 bg-rose-500 rounded-full border-2 border-white"></span>
            </button>
