<aside class="hidden lg:flex fixed left-0 top-0 h-full w-28 bg-maroon-950 flex-col items-center py-8 z-50 border-r border-white/5 shadow-2xl">
    ...
    <div class="flex flex-col gap-3 flex-1 w-full px-3">
        ...
        <a href="{{ route('pembimbing.dashboard') }}" title="Beranda"
           class="group nav-transition flex flex-col items-center justify-center py-4 rounded-[2rem] {{ request()->routeIs('pembimbing.dashboard') ? 'bg-white text-maroon-950 shadow-xl scale-105' : 'text-white/75 hover:bg-white/10 hover:text-white' }}">
            ...
            <span class="text-[9px] font-black uppercase tracking-[0.2em] mt-2 group-hover:opacity-100 transition-opacity {{ request()->routeIs('pembimbing.dashboard') ? 'opacity-100' : 'opacity-80' }}">Beranda</span>
        </a>

        <!-- 2. Data Siswa Bimbingan -->
        <a href="{{ route('pembimbing.data-siswa.index') }}" title="Data Siswa Bimbingan"
           class="group nav-transition flex flex-col items-center justify-center py-4 rounded-[2rem] {{ request()->routeIs('pembimbing.data-siswa.*') ? 'bg-white text-maroon-950 shadow-xl scale-105' : 'text-white/75 hover:bg-white/10 hover:text-white' }}">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="18" x="3" y="3" rx="2"/><circle cx="12" cy="10" r="3"/><path d="M7 21v-2a2 2 0 0 1 2-2h6a2 2 0 0 1 2 2v2"/></svg>
            <span class="text-[9px] font-black uppercase tracking-[0.2em] mt-2 group-hover:opacity-100 transition-opacity {{ request()->routeIs('pembimbing.data-siswa.*') ? 'opacity-100' : 'opacity-80' }}">Data Siswa</span>
        </a>
        ...
        <a href="{{ route('pembimbing.presensi-siswa.index') }}" title="Riwayat Presensi Siswa"
           class="group nav-transition flex flex-col items-center justify-center py-4 rounded-[2rem] {{ request()->routeIs('pembimbing.presensi-siswa.*') ? 'bg-white text-maroon-950 shadow-xl scale-105' : 'text-white/75 hover:bg-white/10 hover:text-white' }}">
            ...
            <span class="text-[9px] font-black uppercase tracking-[0.2em] mt-2 group-hover:opacity-100 transition-opacity {{ request()->routeIs('pembimbing.presensi-siswa.*') ? 'opacity-100' : 'opacity-80' }}">Presensi</span>
        </a>

        <!-- 4. Riwayat Jurnal Logbook -->
        <a href="{{ route('pembimbing.monitoring.index') }}" title="Riwayat Jurnal Logbook"
           class="group nav-transition flex flex-col items-center justify-center py-4 rounded-[2rem] {{ request()->routeIs('pembimbing.monitoring.*') ? 'bg-white text-maroon-950 shadow-xl scale-105' : 'text-white/75 hover:bg-white/10 hover:text-white' }}">
            ...
            <span class="text-[9px] font-black uppercase tracking-[0.2em] mt-2 group-hover:opacity-100 transition-opacity {{ request()->routeIs('pembimbing.monitoring.*') ? 'opacity-100' : 'opacity-80' }}">Logbook</span>
        </a>
