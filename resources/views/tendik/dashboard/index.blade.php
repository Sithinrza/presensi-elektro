        ...
        <form action="{{ route('tendik.lengkapi.profil') }}" method="POST" class="p-5 sm:p-8 space-y-5 sm:space-y-6 overflow-y-auto no-scrollbar">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
                ...
                <div class="space-y-1.5 sm:space-y-2">
                    ...
                    <input type="date" name="tanggal_lahir" max="{{ date('Y-m-d') }}" required value="{{ $tendik->tanggal_lahir ?? '' }}" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 sm:px-4 py-2.5 sm:py-3 text-[13px] sm:text-sm font-bold text-slate-800 focus:ring-2 focus:ring-maroon-500 outline-none">
                </div>
