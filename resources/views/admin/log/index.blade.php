...
<main class="p-10 space-y-8 animate-in">
    ...
    <section class="bg-white rounded-[2.5rem] p-8 border border-maroon-50 shadow-premium">
        ...
        <form id="filterForm" action="{{ route('admin.log') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">

            <div class="space-y-2">
                <label class="text-[10px] font-black text-maroon-900 uppercase tracking-widest ml-1">Cari Siswa</label>
                <div class="relative group">
                    <input type="text" name="search" id="searchInput" value="{{ request('search') }}" placeholder="Nama atau NIS..." class="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 pl-10 text-xs font-bold text-maroon-950 outline-none focus:ring-2 focus:ring-maroon-500 transition-all placeholder:text-slate-300">
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-maroon-500 transition-colors"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>
                </div>
            </div>

            <div class="space-y-2">
                <label class="text-[10px] font-black text-maroon-900 uppercase tracking-widest ml-1">Status Magang</label>
                <select name="status_akun" onchange="submitForm()" class="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 text-xs font-bold text-maroon-950 outline-none cursor-pointer focus:ring-2 focus:ring-maroon-500 appearance-none">
                    <option value="all" {{ request('status_akun') == 'all' ? 'selected' : '' }}>Semua Peserta</option>
                    <option value="Aktif" {{ request('status_akun') == 'Aktif' ? 'selected' : '' }}>Hanya Aktif</option>
                    <option value="Nonaktif" {{ request('status_akun') == 'Nonaktif' ? 'selected' : '' }}>Nonaktif / Selesai</option>
                </select>
            </div>

            <div class="space-y-2">
                <label class="text-[10px] font-black text-maroon-900 uppercase tracking-widest ml-1">Status Validasi</label>
                <select name="status" onchange="submitForm()" class="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 text-xs font-bold text-maroon-950 outline-none cursor-pointer focus:ring-2 focus:ring-maroon-500 appearance-none">
            ...
            <div class="space-y-2">
                ...
                <select name="bulan" onchange="submitForm()" class="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 text-xs font-bold text-maroon-950 outline-none cursor-pointer focus:ring-2 focus:ring-maroon-500 appearance-none">
                    ...
                    <tr class="hover:bg-maroon-50/20 transition-all duration-200 group">
                        <td class="px-10 py-6">
                            <div class="flex items-center gap-4">
                                <div class="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center font-black text-lg text-maroon-900 shrink-0 border-2 border-white group-hover:border-maroon-100 transition-all shadow-inner overflow-hidden relative">
                                    @if($log->foto_profil)
                                ...
                                <div>
                                    <div class="flex items-center gap-1.5 mb-1">
                                        <p class="text-sm font-extrabold text-slate-800 leading-tight group-hover:text-maroon-700 transition-colors">{{ $log->nama_siswa }}</p>
                                        @if($log->status_akun == 'Aktif')
                                            <span class="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse" title="Siswa Aktif"></span>
                                        @else
                                            <span class="w-1.5 h-1.5 bg-slate-400 rounded-full" title="Siswa Nonaktif/Selesai"></span>
                                        @endif
                                    </div>
                                    <p class="text-[9px] font-bold text-slate-400 uppercase tracking-tighter italic">
...
<script>
    ...
    // Fitur Debounce untuk Input Text (Nunggu admin berhenti ngetik 0.8 detik baru auto-submit)
    let typingTimer;
    const inputElement = document.getElementById('searchInput');

    inputElement.addEventListener('keyup', function () {
        clearTimeout(typingTimer);
        typingTimer = setTimeout(submitForm, 800); // 800ms delay
    });
