...
<main class="p-10 space-y-8 animate-in">
    ...
    <form action="{{ route('admin.data.tendik.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-[3rem] shadow-premium overflow-hidden border border-maroon-50">
        ...
        <div class="p-8 md:p-10 space-y-10">
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50 p-6 rounded-3xl border border-slate-200">
                    ...
                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">
                            Unit Kerja / Prodi <span class="text-rose-500 text-sm leading-none align-top">*</span>
                        </label>
                    ...
                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">
                            Pangkat / Golongan <span class="text-rose-500 text-sm leading-none align-top">*</span>
                        </label>
            ...
            <div>
                <div class="flex items-center gap-3 border-b border-maroon-50 pb-4 mb-6">
                    ...
                    <div>
                        <h3 class="text-lg font-black text-slate-800 tracking-tight uppercase italic leading-none">Detail Profil & Struktural</h3>
                        <p class="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-widest">KOLOM OPSIONAL</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-transparent p-6 rounded-3xl border-2 border-dashed border-slate-300">
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="id_pend_terakhir" class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3.5 text-sm font-bold text-slate-800 focus:ring-2 focus:ring-maroon-500 outline-none transition-all cursor-pointer shadow-sm appearance-none">
                            ...
                            @foreach($pendidikan as $pd)
                                <option value="{{ $pd->id_pend_terakhir }}" {{ old('id_pend_terakhir') == $pd->id_pend_terakhir ? 'selected' : '' }}> {{ $pd->name }} </option>
                            @endforeach
                        </select>
                    </div>
                    <div class="space-y-2">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="tel" inputmode="numeric" oninput="this.value = this.value.replace(/[^0-9]/g, '')" name="no_hp" value="{{ old('no_hp') }}" placeholder="08..." class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3.5 text-sm font-bold text-slate-800 focus:ring-2 focus:ring-maroon-500 outline-none transition-all shadow-sm">
                    </div>
                ...
                <div class="p-4 bg-amber-50 rounded-2xl border border-amber-100 flex items-start gap-3 mt-6">
                    <div class="w-8 h-8 bg-amber-500 text-white rounded-lg flex items-center justify-center shrink-0 shadow-sm mt-0.5">
                    ...
                    <p class="text-[10px] font-bold text-amber-800 leading-relaxed italic uppercase mt-0.5">
                        Catatan: Bagian Detail Profil bersifat opsional dan dapat dilengkapi oleh Tendik terkait secara mandiri di kemudian hari.
                    </p>
                </div>
            </div>
        ...
        <div class="bg-maroon-50/50 border-t border-maroon-100 p-8 flex justify-end gap-4">
            <a href="{{ route('admin.data.tendik.index') }}" class="bg-white text-maroon-900 border border-maroon-100 py-4 px-10 rounded-2xl font-black text-xs uppercase tracking-widest text-center shadow-sm hover:bg-maroon-50 active:scale-95 transition-all">Batalkan</a>
            <button type="submit" class="bg-maroon-950 text-white py-4 px-10 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl hover:bg-maroon-800 active:scale-95 transition-all flex items-center gap-2">
