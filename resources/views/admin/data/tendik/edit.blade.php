...
<main class="p-6 lg:p-10 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-black text-slate-800 tracking-tight leading-none">Edit Data Tendik</h1>
                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1.5">Pembaruan Master Data Personel</p>
            </div>
        </div>
        <div class="px-4 py-2 bg-amber-50 border border-amber-100 rounded-xl w-fit">
    ...
    <form action="{{ route('admin.data.tendik.update', $t->id_tendik ?? 1) }}" method="POST" class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
        @csrf
        @method('PUT')
        ...
        <div class="p-8 md:p-10 space-y-12">
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50/50 p-6 lg:p-8 rounded-3xl border border-slate-100">
                    ...
                    <div class="space-y-2 md:col-span-2">
                        ...
                        <select name="status" required class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3.5 text-sm font-bold text-slate-800 focus:ring-2 focus:ring-maroon-500 focus:border-maroon-500 outline-none transition-all cursor-pointer shadow-sm appearance-none">
                            <option value="Aktif" {{ (old('status', $t->status ?? '') == 'Aktif') ? 'selected' : '' }}>Aktif Bekerja</option>
                            <option value="Nonaktif" {{ (old('status', $t->status ?? '') == 'Nonaktif') ? 'selected' : '' }}>Nonaktif (Pensiun/Cuti)</option>
                        </select>
                    </div>

                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Nama Lengkap & Gelar</label>
                        <input type="text" name="nama_lengkap" value="{{ old('nama_lengkap', $t->nama_lengkap ?? '') }}" required class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3.5 text-sm font-bold text-slate-800 focus:ring-2 focus:ring-maroon-500 focus:border-maroon-500 outline-none transition-all shadow-sm placeholder:text-slate-300">
                    </div>
                    
                    <div class="space-y-2">
                        <label class="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Email Akun</label>
                        <input type="email" name="email" value="{{ old('email', $t->user->email ?? '') }}" required class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3.5 text-sm font-bold text-slate-800 focus:ring-2 focus:ring-maroon-500 focus:border-maroon-500 outline-none transition-all shadow-sm placeholder:text-slate-300">
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="id_unit_kerja" required class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3.5 text-sm font-bold text-slate-800 focus:ring-2 focus:ring-maroon-500 focus:border-maroon-500 outline-none transition-all cursor-pointer shadow-sm appearance-none">
                            ...
                            @foreach($unit_kerja ?? [] as $uk)
                                <option value="{{ $uk->id_unit_kerja }}" {{ (old('id_unit_kerja', $t->id_unit_kerja ?? '') == $uk->id_unit_kerja) ? 'selected' : '' }}>
                                    {{ $uk->nama_unit }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="space-y-2 md:col-span-2">
                        ...
                        <input type="password" name="password" minlength="6" placeholder="Biarkan kosong jika tidak ingin mengubah password..." class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3.5 text-sm font-bold text-slate-800 focus:ring-2 focus:ring-maroon-500 focus:border-maroon-500 outline-none transition-all shadow-sm placeholder:text-slate-400">
                        <p class="text-[9px] font-bold text-amber-500 ml-1 mt-1.5 uppercase tracking-widest flex items-center gap-1">
                            ...
                            Hanya isi jika ingin mereset password pengguna ini.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div>
                <div class="flex items-center gap-4 border-b border-slate-100 pb-5 mb-6">
                    ...
                    <div>
                        <h3 class="text-lg font-black text-slate-800 tracking-tight leading-none">Detail Profil Tambahan</h3>
                        <p class="text-[10px] font-bold text-slate-400 mt-1.5 uppercase tracking-widest">Atribut Pelengkap</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white border border-slate-100 p-6 lg:p-8 rounded-3xl">
                    <div class="space-y-2">
                        ...
                        <input type="text" name="nip" value="{{ old('nip', $t->nip ?? '') }}" placeholder="Kosongkan jika belum tahu..." class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3.5 text-sm font-bold text-slate-800 focus:bg-white focus:ring-2 focus:ring-maroon-500 focus:border-maroon-500 outline-none transition-all shadow-sm placeholder:text-slate-300">
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="id_agama" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3.5 text-sm font-bold text-slate-800 focus:bg-white focus:ring-2 focus:ring-maroon-500 focus:border-maroon-500 outline-none transition-all cursor-pointer shadow-sm appearance-none">
                            <option value="" disabled {{ old('id_agama', $t->id_agama ?? '') ? '' : 'selected' }}>Belum diisi...</option>
                            @foreach($agama ?? [] as $item)
                                <option value="{{ $item->id_agama }}" {{ (old('id_agama', $t->id_agama ?? '') == $item->id_agama) ? 'selected' : '' }}>
                                    {{ $item->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="space-y-2 md:col-span-2">
                        ...
                        <select name="id_jabatan" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3.5 text-sm font-bold text-slate-800 focus:bg-white focus:ring-2 focus:ring-maroon-500 focus:border-maroon-500 outline-none transition-all cursor-pointer shadow-sm appearance-none">
                            <option value="" disabled {{ old('id_jabatan', $t->id_jabatan ?? '') ? '' : 'selected' }}>Belum diisi...</option>
                            @foreach($jabatan ?? [] as $jb)
                                <option value="{{ $jb->id_jabatan }}" {{ (old('id_jabatan', $t->id_jabatan ?? '') == $jb->id_jabatan) ? 'selected' : '' }}>
